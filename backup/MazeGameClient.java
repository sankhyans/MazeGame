...
public class MazeGameClient extends UnicastRemoteObject implements ClientConnect{
    ...
    public static MazeGame gameState = null;
    ...
    public MazeGameClient(int id, String name, int port, long initTime) throws RemoteException
    {
        super();
        player_id= id;
        player_name = name;
        gameState = new MazeGame();

        try
        {
            String host = "//127.0.0.1:"+port+"/server";
            clientStub = (MazeGameP2PInterface) Naming.lookup(host);

            Registry registrySub = LocateRegistry.getRegistry("127.0.0.1",port);
            clientlifeStub = (ClientLifeChecker) registrySub.lookup("ClientLife");
            //clientBootStub = (BootstrapperInterface) registrySub.lookup("bootstrap");
            init(initTime);
    ...
    public void init(long initTime) throws RemoteException
    {
        ...
        try{
            ...
            else
            {
              ...
              try
              {
                if(initTime != 0)
                {
                    while(System.currentTimeMillis() - initTime <20)
                    {
                        Thread.sleep();
                    }
                }
    ...
    public static void playGame() throws RemoteException
    {
        ...
        {
            System.out.println("playGame:Server has died.");
            System.exit(0);
        }
        catch (java.rmi.UnmarshalException exp)
        {
